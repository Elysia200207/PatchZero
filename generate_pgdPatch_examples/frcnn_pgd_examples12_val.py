...
class PatchAttack:
    ...
    def attack(self, input, target):
        ...
        os.makedirs('./adv_examples2012/val/cleans', exist_ok=True)    
        os.makedirs('./adv_examples2012/val/images', exist_ok=True)
        os.makedirs('./adv_examples2012/val/labels', exist_ok=True)
        os.makedirs('./adv_examples2012/val/masks', exist_ok=True)
        ...
        for i in range(b):
            ...
            save_image(images[i], f'./adv_examples2012/val/cleans/{clean_filename}')
            save_image(adv_images[i], f'./adv_examples2012/val/images/{image_filename}')
            save_image(msk[i], f'./adv_examples2012/val/masks/{mask_filename}')
            label_info = {
        ...
        # 将标签信息保存到一个JSON文件中
        with open('./adv_examples2012/val/labels.json', 'a') as label_file:
...
def main():
    ...
    dataset= VOCDetection('/gpfs/hulab/huangyifan/datasets', year='2012', image_set='val', download=False, transform=data_augmentation)
